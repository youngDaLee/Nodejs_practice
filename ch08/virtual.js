...
function connectDB() {
	// 데이터베이스 연결 정보
	var databaseUrl = 'mongodb://localhost:27017/node';
...
function doTest() {
	...
	var user = new UserModel({"info":'myId3 myName'});
	...
	UserModel.find({}, function(err, results) {
		...
		if (results) {
			results.forEach(function(element, index){
				console.log('user %d ->  id : %s, name : %s', index,  element._doc.id, element._doc.name);
			})
